...
<div class="p-6">
    <div class="flex justify-between items-center mb-4">
        ...
        <a href="{{ url('PmbMstPendaftarans/master-sekolah/add/' . str_replace(' ', '-', $role ?? 'admin')) }}" 
                ...
                @forelse ($sekolah as $index => $item)
                    <tr>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            {{ $loop->iteration + ($sekolah->currentPage() - 1) * $sekolah->perPage() }}
                        </td>
                        ...
                        <td class="px-6 py-4 text-sm">
                            <a href="{{ url('PmbMstPendaftarans/master-sekolah/edit/' . str_replace(' ', '-', $role ?? 'admin') . '/' . $item->id) }}" 
                               class="text-blue-600 hover:text-blue-800 mr-3">
                                Edit
                            </a>
                            <form action="{{ url('PmbMstPendaftarans/master-sekolah/delete/' . str_replace(' ', '-', $role ?? 'admin') . '/' . $item->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
