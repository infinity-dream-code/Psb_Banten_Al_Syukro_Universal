...
<div class="max-w-2xl mx-auto mt-5">
  <div class="bg-white border rounded-xl">
    <div class="border-b px-6 py-4 text-base font-semibold">Edit Role</div>
    <div class="p-6">
      @if(session('success'))
      ...
      <form method="POST" action="{{ route('master.role.update', $role->id) }}">
        @csrf
        @method('PUT')
        <div class="mb-5">
        ...
        <div class="mb-5">
          ...
          <div class="grid grid-cols-2 gap-2">
            @php
              ...
              $selectedMenus = is_array($role->menu) ? $role->menu : json_decode($role->menu, true);
            @endphp
            @foreach($menus as $menu)
              <label class="inline-flex items-center space-x-2">
                <input type="checkbox" name="menu[]" value="{{ $menu }}" class="form-checkbox h-5 w-5 text-green-600 rounded"
                       {{ is_array($selectedMenus) && in_array($menu, $selectedMenus) ? 'checked' : '' }}>
