...
<div class="max-w-2xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-5">
    ...
    <form action="{{ route('informasi.update', $informasi->id) }}" method="POST" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="informasi" id="informasi" rows="4"
                      class="block w-full border border-gray-300 rounded-lg px-3 py-2 shadow-sm text-gray-700
                             focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500">{{ old('informasi', $informasi->informasi) }}</textarea>
        </div>
