...
<div class="max-w-2xl mx-auto bg-white shadow-lg rounded-xl p-8 mt-5">
    ...
    {{-- Form Upload Brosur --}}
    <form action="{{ route('setting-brosur.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        <div>
            <label for="brosur" class="block text-sm font-medium text-gray-700 mb-2">
                Upload Brosur (PDF / Gambar)
            </label>
