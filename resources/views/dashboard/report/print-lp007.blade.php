...
<head>
    ...
    <title>Daftar Pendaftar Lunas Daftar Ulang{{ $tahun }}</title>
    <style>
...
<div class="container {{ $page > 0 ? 'page-break' : '' }}">
    ...
    <div class="title-section">Daftar Peserta Lunas Registrasi Ulang<br>{{ $tahun }}</div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $p->jurusan }}</td>
                <td>{{ $p->no_hp }}</td>
                <td>{{ $p->tgl_bayar_regis ? \Carbon\Carbon::parse($p->tgl_bayar_regis)->format('d M Y H:i') : '-' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
